...
without_sources: false             # Загрузить без исходников (*.rpy исключаются, остаются только *.rpyc)
heroes:                            # Теги "Герои" из встроенного загрузчика ЕЛ
  - "Лена"
  - "Славя"
  - "Ульяна"
  - "Мику"
...
genres:                            # Теги "Жанры"
  ...
  - "Драма"
  - "Приключения"
mod_type:                          # Теги "Тип мода"
  - "С выборами"
tags: []                           # Дополнительные произвольные теги
...
* `project_name`: название мода, будет использоваться для формирования пути, куда копируются файлы репозитория для проверки. Например, если мод лежит по пути `Everlasting Summer/game/MyMod`, то в качестве значения параметра `project_name` нужно указать `MyMod`
* `without_sources`: аналог чекбокса «Загрузить без исходников» из встроенного загрузчика ЕЛ. При `true` из загрузки исключаются все `*.rpy` файлы — в мастерскую уезжают только скомпилированные `*.rpyc`. По умолчанию `false`
* `heroes` / `genres` / `mod_type` / `tags`: списки тегов, которые проставляются предмету в Steam Workshop. Соответствуют категориям «Герои», «Жанры», «Тип мода» из встроенного редактора тегов ЕЛ; `tags` — для произвольных дополнительных тегов. Все четыре списка опциональны и в итоговом `workshop.vdf` объединяются (с удалением дубликатов) в один блок `tags`.

  В YAML можно писать русские названия — как в редакторе тегов («Лена», «Фантастика», «С выборами» и т.д.), скрипт сам переведёт их в формат, который Steam хранит у себя (`Lena`, `Sci-fi`, `Variative`). Неизвестные значения отдаются в Workshop как есть — это позволяет указывать любые свои теги по-английски.

  Полный список поддерживаемых значений (как в редакторе тегов / как уходит в Steam):

  **Герои:** `Алиса` → `Alisa`, `Лена` → `Lena`, `Славя` → `Slavya`, `Ульяна` → `Ulyana`, `Юля` → `Yulya`, `Мику` → `Miku`, `Женя` → `Zhenya`, `Ольга Дмитриевна` → `Olga Dmitrievna`, `Семён` → `Semyon`, `Электроник` → `Electronik`, `Шурик` → `Shurik`, `Маша` → `Masha`, `Виола` → `Viola`, `Пионер` → `Pioneer`, `Новый персонаж` → `New character`.

  **Жанры:** `Фантастика` → `Sci-fi`, `Мистика` → `Mystic`, `Повседневность` → `Slice of life`, `Романтика` → `Romance`, `Комедия` → `Comedy`, `Детектив` → `Mystery`, `Драма` → `Drama`, `Экшн` → `Action`, `Хоррор` → `Horror`, `Трэш` → `Trash`, `Приключения` → `Adventure`, `Триллер` → `Thriller`.

  **Тип мода:** `Линейный` → `Linear`, `С выборами` → `Variative`, `Технический` → `Technical`.

  Английские названия и их соответствие категориям сверены с бинарём `ES_Content_Uploader.exe` (Steam Workshop tag-enum'ы), который зашивает в `.tagsHistory` именно такие строки.
